 ...
 * Examples:
 * 
 * Input : n = 12 Output : 13 
 * // We break n = 12 in three parts {12/2, 12/3,
 * 12/4} /
 * / = {6, 4, 3}, now current sum is = (6 + 4 + 3) = 13 
 * // again we break
 * 6 = {6/2, 6/3, 6/4} = {3, 2, 1} = 3 + 
 ...
 *
 */
public class BreakSumInThreeParts
{
	public static void breakSum(int sum)
	{
		...
		for (int i = 2; i <= sum; i++)
		{
			table[i] = Math.max(table[i / 2] + table[i / 3] + table[i / 4], i);
		}
